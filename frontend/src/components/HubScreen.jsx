  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white/80 backdrop-blur-sm p-6 sm:p-8 rounded-2xl shadow-2xl text-center">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
        ...
        <Link
          to="/leaderboard"
          className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-6 rounded-lg text-lg shadow-lg"
        >
          🏆 View Leaderboard
        </Link>
      </div>
        ...
        {riddles.map((riddle, index) => {
          ...
          return (
            <Link
              to={destination}
              key={riddle.id}
              className={`aspect-square flex flex-col items-center justify-center p-4 rounded-xl shadow-lg transform hover:scale-105 transition-all ${
                riddle.isRiddleSolved
